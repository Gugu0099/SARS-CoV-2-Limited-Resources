...
# MATLAB Parula colormap data (cm_data) should be defined before this point
...
# Create a colormap from the Parula data
...
boundaries = [0, 1.6, 2.5, 4, 6, 10, 16, 25, 30]
norm = BoundaryNorm(boundaries, ncolors=parula_cmap.N, clip=True)
# Normalize the color scale to match the distribution in the image
...
ax = sns.heatmap(pivot_table, annot=False, mask=mask, cmap=parula_cmap, norm=norm, cbar_kws={'label': 'Viral cycle time (hr)', 'ticks': boundaries})
